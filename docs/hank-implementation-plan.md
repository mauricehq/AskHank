...
- [x] `formatNudgePrompt()` injects structured YAML into system prompt (previous_item, price, date)
...
| Verdict summary | Hank-voice 1-2 sentence insight for the share card, generated by Call 3 (`verdictSummary`) |

**Verdict summary generation:** Call 3 is a dedicated LLM call after the closing line is generated. `buildVerdictSummaryPrompt()` gives the LLM conversation context + closing line and asks for a standalone 1-2 sentence summary for the share card. Fallback: if Call 3 fails, the share card insight section is hidden.

**Implementation:**
- [ ] Add `shareToken` field to conversations table (generated on verdict)
- [x] Add `verdictSummary` field — generated by Call 3 after closing, saved with verdict
