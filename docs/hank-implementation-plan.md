...
- [x] On close: save closing line via forced `closing_response` tool call on Call 2 closing turns. Call 3 generates `verdictSummary` for share cards. Fallback: if tool call fails, closing line falls back to `call2.content`.
...
- [x] `selectMemoryNudge()` picks ONE past conversation to reference — filters to same category, valid item, not "other", must have a verdict (only closed conversations); sorts by lowest reference count then most recent
- [x] `formatNudgePrompt()` injects structured YAML into system prompt (item, price, date, verdict, reason) with outcome-aware guidance (denied = reinforce skepticism, approved = acknowledge but bar stays high)
...
### 5b: Share System ✅ (Verdict Card shipped, Roast + Saved deferred)
...
Dark card with DENIED/APPROVED stamp as visual hero, item name, price, category, Hank's verdict summary, and "Your case" score bar (0–100). Container query CSS renders both portrait (4:5 for download, 1080×1350) and landscape (1.9:1 for OG, 1200×630) from the same component. Content density calculator adjusts text sizing based on character length.
...
| Verdict summary | Hank-voice 1-2 sentence insight, generated by Call 3 (`verdictSummary`) |
| Score bar | `shareScore` — debate strength mapped to 5–99 scale |

**Verdict summary generation:** Call 3 is a dedicated LLM call after the closing line is generated. `buildVerdictSummaryPrompt()` gives the LLM conversation context + closing line and asks for a standalone 1-2 sentence summary. Fallback chain with retry for length. If Call 3 fails entirely, the share card insight section is hidden.

**Implementation (all done):**
- [x] `shareCards` table in Convex — token-based (22-char base62), decoupled from conversations table. Stores snapshot of verdict data + image URLs.
- [x] `createVerdictCard` mutation — auth-gated, idempotent (returns existing token if already created)
- [x] `VerdictShareCard` component — container query CSS, portrait + landscape, three text density tiers
- [x] `ShareCardModal` — portal-based, live preview, "Copy Link" (native share on mobile, clipboard on desktop) + "Download Image" (two-step iOS Safari safe)
- [x] Share button on `VerdictCard` fully wired — creates card, opens modal
- [x] Public route `/card/[token]` — SSR with full OG/Twitter metadata
- [x] OG redirect route `/api/og/[token]` — edge runtime, 302 redirect to Vercel Blob JPEG
- [x] Image generation API `/api/card-images/[cardType]` — Puppeteer pipeline, auth + rate limiting (5/min), SSRF guard, Vercel Blob upload, caching
- [x] Render routes `/render/[cardType]/[token]` — secret-gated pages used exclusively by Puppeteer, dual format (download/og)
- [x] `screenshot.server.ts` — `puppeteer-core` + `@sparticuz/chromium` for serverless, SSRF allowlist, JPEG 90% quality at 2x DPR
- [x] Card registry pattern (`src/lib/cards/registry.ts`) — extensible for future card types
- [x] Security: ownership verification before Puppeteer, render secret header, Vercel Blob hostname validation on URL writes, token format regex validation

**Design reference:** `docs/sharing-v2.md` — final design decisions for layouts, score mapping formula, stamp hero approach.
...
- [ ] Roast card visual component + registry entry (schema + types already support `"roast"` card type)
- [ ] Optimized for vertical format (1080x1350) — Instagram Stories / TikTok friendly

#### "Saved $X" Share Card (deferred)

The savings counter as a personal achievement card — Strava miles, Duolingo streaks energy. Schema + types already support `"savedTotal"` card type.

- [ ] Shareable stats card visual component + registry entry
- [ ] Accessible from stats page (share button)

#### Clean Mode (for screen recording, deferred)

Screen recording is the primary TikTok/Reels format — users record themselves arguing with Hank in real time. The app just needs to look good on camera.
...
### 5c: Landing Page ✅
- [x] Full landing page with 8 sections: Hero, ChatDemo, HowItWorks, WhyHankWorks, Scorecard, FreeToTry, FairWarning, FinalCTA
- [x] Navbar with mobile-responsive hamburger menu
- [x] Footer with links
- [x] Interactive chat demo with autoplay conversation
- [x] Problem-first narrative copy
- [x] SEO foundations: `robots.ts`, `sitemap.ts`, OpenGraph metadata
...
| 5: Polish + Share | ✅ 5a-5c done | Verdict share card (full pipeline), landing page, SEO. Roast card + Saved card + Clean Mode deferred |
...
The path to bangers: ~~**2e** (signature moves)~~ ✅ → ~~**5b** (shareable cards)~~ ✅ → **7** (dossier) → **8** (banger system). Signature moves are live. Verdict cards are shareable. The dossier gives Hank memory. Bangers engineer the moments worth sharing.
