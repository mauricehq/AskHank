...
export function useConversation() {
  ...
  return { messages, isThinking, isError, send, reset, verdict, conversationId, loadConversation, item: conversation?.item, estimatedPrice: conversation?.estimatedPrice, category: conversation?.category, excuse: conversation?.excuse, verdictTagline: conversation?.verdictTagline, verdictSummary: conversation?.verdictSummary, outOfCredits, thinkingSince: conversation?.thinkingSince ?? null };
}
